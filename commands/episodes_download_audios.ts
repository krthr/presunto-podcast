...
export default class EpisodesDownloadAudios extends BaseCommand {
  static commandName = 'episodes:download-audios'
  static description = ''
  ...
  async run(audioService: AudioService) {
    ...
    while (episodes.length) {
      ...
      const promises = chunks.map(async (episode) => {
        logger.info(
          `processing episode ${episode.title} with id=${episode.acastEpisodeId} slug=${episode.slug}`
        )
